...
def eval_icl_performance(model, scales=[1.0, 4.0, 9.0]):
    results = {}
    for scale in scales:
        metrics = eval_model(
            model,
            ...
            data_name="gaussian",
            data_sampler_kwargs={"random_cov": True, "cov_scale": scale}
        )
...
def build_evals(conf):
    ...
    # ICL evaluations with different covariance scales
    for scale in [1.0, 4.0, 9.0]:
        evaluation_kwargs[f"random_cov_scale_{scale}"] = {
            "data_sampler_kwargs": {
                "random_cov": True,
                "cov_scale": scale
            },
    ...
    # Fixed covariance evaluation
    evaluation_kwargs["fixed_cov"] = {
        "data_sampler_kwargs": {
            "random_cov": False,
            "scale": torch.eye(n_dims)
        },
    }
    
    # Add evaluations for different context lengths
    for n_ctx in [40, 70, 100]:
        evaluation_kwargs[f"ctx_length_{n_ctx}"] = {
            "n_points": n_ctx
        }

    # Merge base_kwargs with specific evaluation settings
...
def eval_icl_performance(model, task_name, data_name, n_dims, batch_size=64, num_eval_examples=1280):
    """Evaluate in-context learning performance under different conditions"""
    all_metrics = {}
    
    # Evaluate with different scales of random covariance
    for scale in [1.0, 4.0, 9.0]:
        data_sampler_kwargs = {
            "random_cov": True,
            "cov_scale": scale
        ...
        all_metrics[f"random_cov_scale_{scale}"] = metrics
    
    # Evaluate with fixed identity covariance
    metrics = eval_model(
        model=model,
        task_name=task_name,
        data_name=data_name,
        ...
        batch_size=batch_size,
        num_eval_examples=num_eval_examples,
        data_sampler_kwargs={"random_cov": False, "scale": torch.eye(n_dims)}
    )
    all_metrics["fixed_cov"] = metrics
...
def compute_evals(all_models, evaluation_kwargs, save_path=None, recompute=False):
    ...
    for eval_name, kwargs in tqdm(evaluation_kwargs.items()):
        ...
        for model in all_models:
            ...
            if "random_cov_scale" in eval_name or eval_name == "fixed_cov":
                metrics[model.name] = eval_icl_performance(
                    model,
                    kwargs["task_name"],
                    kwargs["data_name"],
                    kwargs["n_dims"],
